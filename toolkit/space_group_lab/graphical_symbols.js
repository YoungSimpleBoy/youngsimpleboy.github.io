(function (global) {
    ...
    //    })
    // 4) drawPrimitiveCircle(ctx, { x, y, radius, stroke, lineWidth })
    // 5) drawGuideAxes(ctx, { x, y, radius, stroke, lineWidth, dash })
    ...
    function drawPrimitiveCircle(ctx, options) {
        const opts = options || {};
        const x = Number(opts.x) || 0;
        const y = Number(opts.y) || 0;
        const radius = Math.max(1, Number(opts.radius) || 1);
        const stroke = opts.stroke || "rgba(17, 24, 39, 0.9)";
        const lineWidth = Number(opts.lineWidth) || 1.1;
    ...
    function drawGuideAxes(ctx, options) {
        const opts = options || {};
        const x = Number(opts.x) || 0;
        const y = Number(opts.y) || 0;
        const radius = Math.max(1, Number(opts.radius) || 1);
        const stroke = opts.stroke || "rgba(17, 24, 39, 0.28)";
        const lineWidth = Number(opts.lineWidth) || 0.86;
        const dash = Array.isArray(opts.dash) ? opts.dash : [3, 3];
